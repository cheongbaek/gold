...
GAIN_TABLE = [
    ...
    (2.2,     2.3,  0.20,  0.21),  # LFD 3.2  : ★1/5카에서는 도달 불가였던 구간★
    # ══════════════════════════════════════════════════════════════════════════
    #  ★★ [2026-08-05] kasa 고속행 신설 (2.65 ~ 4.42 m/s) — ★전부 미검증 추정값★ ★★
    # ══════════════════════════════════════════════════════════════════════════
    #  왜 필요한가 : max_speed_ms 를 5펄스(4.42 m/s = 15.9 km/h)로 올렸는데 표의 마지막
    #    행이 2.2 였다. lookup 은 마지막 행으로 클램프하므로 2.2~4.42 구간 전체가
    #    2.2 행의 게인을 쓰게 된다(스케줄이 끊긴다).
    #
    #  ★게인은 일부러 올리지 않았다 (2.2 행 값을 그대로 유지)★
    #    · 이 표의 설계철학이 "고속대는 Kp 유지(과게인 방지), 감쇠는 LFD·lead 가 맡는다"다.
    #    · 실차 로스백 없이 Kp 를 선형 연장하면(추세 +0.5/[m/s] → 4.42 에서 3.4) 1톤급
    #      유인차량에서 과게인으로 발산할 수 있다. 올리는 쪽이 위험한 방향이다.
    #    · 즉 값 자체는 클램프와 같지만, ★결정을 눈에 보이게 적어두는 것★이 목적이다.
    #      (클램프는 조용히 일어나서 "왜 고속에서 게인이 안 변하나"를 추적할 수 없다)
    #
    #  ⚠️ 실차 검증 전에는 max_speed_ms 를 단계적으로 올릴 것 :
    #       3펄스(2.65) → 로스백 확인 → 4펄스(3.54) → 확인 → 5펄스(4.42)
    #     휠베이스가 0.73 → 1.25m 로 커져 같은 조향각의 회전반경이 1.7배다. 이 표는
    #     애초에 그 전제로 튜닝된 게 아니므로 어차피 전 구간 재튜닝이 필요하다.
    (2.65,    2.3,  0.20,  0.21),  # LFD 3.85  3펄스   9.5 km/h
    (3.10,    2.3,  0.20,  0.21),  # LFD 4.50
    (3.54,    2.3,  0.20,  0.21),  # LFD 5.15  4펄스  12.7 km/h
    (4.00,    2.3,  0.20,  0.21),  # LFD 5.80
    (4.42,    2.3,  0.20,  0.21),  # LFD 6.40  5펄스  15.9 km/h  ← 현 운용상한
...
LFD_TABLE = [
    ...
    (2.2,     3.2),   # ω_n 0.97  ★1/5카에서는 도달 불가였던 구간★
    # ══════════════════════════════════════════════════════════════════════════
    #  ★★ [2026-08-05] kasa 고속행 신설 — ★이 확장은 안전상 필수다★ ★★
    # ══════════════════════════════════════════════════════════════════════════
    #  max_speed_ms 를 5펄스(4.42 m/s)로 올렸는데 표가 2.2 에서 끝나면 lookup_lfd 가
    #  그 값(3.2)으로 클램프한다. 그러면 고속에서 ω_n = v·√2/LFD 가 이렇게 된다:
    #        v=2.65 → 1.17     v=3.54 → 1.56     v=4.42 → ★1.95★  rad/s
    #  ★로스백 실측 임계는 ω_n ≈ 1.2 이고, 그 위에서는 루프지연(≈0.9s) 때문에 유효감쇠가
    #    음(−)이 되어 발산 진동이 난다★ (LFD_TABLE v6.6.2 항목 참고). 즉 표를 그대로 두고
    #    속도만 올리면 고속 구간에서 조향이 발산한다 — 클램프를 방치할 수 없는 이유다.
    #
    #  설계 : ω_n 을 2.2 행과 같은 ≈0.97 로 유지한다 → LFD ≈ 1.455 · v (속도에 비례).
    #    이건 이 표의 원래 설계기준(v6.6.2: "LFD를 속도에 대략 비례시켜 ω_n 을 임계 밑으로
    #    묶는다")을 그대로 고속으로 연장한 것이다. 새 값을 발명한 게 아니다.
    #
    #  ⚠️ 그래도 ★실차 미검증★ 이다. 7/29·7/30 두 번의 LFD 상향 실험이 모두 |CTE| 를
    #     +82~94% 악화시킨 기록이 위에 있다 — 단 그것들은 ★같은 속도에서 LFD 만 늘린★
    #     것이고(동작점 이동), 이번은 ★속도가 올라간 만큼만 비례해서 늘린★ 것이라 성질이
    #     다르다. 그럼에도 코너에서 안쪽을 자를 위험은 같은 방향이므로:
    #       · 곡률캡 min(표, √(1.5·R)) 이 여전히 급코너를 보호한다(완만코너에선 안 걸린다)
    #       · max_speed_ms 를 3 → 4 → 5펄스로 단계적으로 올리며 로스백을 볼 것
    (2.65,    3.85),  # ω_n 0.97   3펄스   9.5 km/h
    (3.10,    4.50),  # ω_n 0.97
    (3.54,    5.15),  # ω_n 0.97   4펄스  12.7 km/h
    (4.00,    5.80),  # ω_n 0.97
    (4.42,    6.40),  # ω_n 0.98   5펄스  15.9 km/h  ← 현 운용상한
...
class DrivingNode(Node):
    ...
    def __init__(self):
        ...
        #      2.65          1~3  (3단계)              9.5 km/h
        #      3.54          1~4  (4단계)             12.7 km/h
        #      4.42          1~5  (5단계)             15.9 km/h  ← ★현재 기본값★
        #      8.84          1~10 (10단계)            31.8 km/h
        #     13.26          1~15 (15단계)            47.7 km/h  ← 장기 목표(차량 상한)
        #
        #  ★★ [2026-08-05] 기본값 2.65 → 4.42 (3펄스 → 5펄스) ★★
        #    금색차는 1/5카와 달리 ★유인운전이 가능하고, 안전을 위해 항상 사람이 타서
        #    브레이크를 밟을 준비를 한다★. 그래서 저속 강제의 전제가 달라졌고, 장기적으로
        #    15펄스까지 올리는 것이 목표다(사용자 결정).
        #
        #    두 게인표에 이 속도까지의 행을 함께 신설했다 — 안 하면 고속 구간 전체가
        #    2.2 행으로 클램프되고 ★LFD 쪽은 ω_n 이 1.95 rad/s 로 실측 불안정 임계(≈1.2)를
        #    넘어 조향이 발산한다★. LFD_TABLE / GAIN_TABLE 의 [2026-08-05] 항목을 볼 것.
        #      · LFD  : ω_n≈0.97 유지 설계(LFD≈1.455·v)로 연장 — 표의 원래 기준을 연장한 것
        #      · GAIN : ★일부러 안 올렸다★ (2.2 행 값 유지). 실차 로스백 없이 Kp 를 올리는
        #               것은 1톤급 유인차량에서 위험한 방향이다.
        #
        #    ⚠️ ★두 표의 고속행은 전부 실차 미검증이다★ 단계적으로 올릴 것:
        #         3펄스(2.65) → 로스백 확인 → 4펄스(3.54) → 확인 → 5펄스(4.42)
        #       현장 튜닝 : ros2 param set /driving_node max_speed_ms 2.65
        #    ⚠️ E-stop 확인시간이 A/B보드 모두 500ms 다 → 4.42 m/s 에서 약 2.2m 진행 후
        #       제동된다(펌웨어 헤더 명시). 사람이 발로 밟는 것이 1차 수단인 전제다.
        #    · 휠베이스가 0.73 → 1.25m 로 커져 같은 조향각의 회전반경이 1.7배가 되었다.
        #      GAIN_TABLE 은 그 전제로 튜닝된 게 아니므로 어차피 실차 재튜닝이 필요하다.
        # ══════════════════════════════════════════════════════════════════════
        self.declare_parameter("max_speed_ms", 4.42)   # 상한[m/s] — 5펄스 ≈ 15.9km/h
        ...
        # ── 하한 override ──────────────────────────────────────────────────────
        #   ★[2026-08-05] 신설★ min_speed_ms 는 원래 상한에 비례해 산출된다
        #   (MIN_SPEED_RATIO = 1/2.8). 상한을 4.42 로 올리면 하한도 1.58 m/s(≈2펄스)로
        #   함께 올라가서 ★최저 주행속도가 5.7 km/h 가 된다★. 저속 시험·수집에서는
        #   1펄스(0.884)를 유지하고 싶을 수 있으므로 직접 지정할 길을 열어 둔다.
        #     0 (기본) = 자동(상한 비례 산출, 종전 동작 그대로)
        #     >0       = 그 값을 하한으로 사용 (MIN_SPEED_FLOOR 와 상한 사이로 클램프)
        #   예: ros2 param set /driving_node min_speed_ms 0.9    (1펄스 유지)
        self.declare_parameter("min_speed_ms", 0.0)
        self._min_speed_override = float(self.get_parameter("min_speed_ms").value)
        ...
        # ★★ [2026-08-05] 4.0 고정 → LFD_TABLE 최상단에서 파생 ★★
        #   [0713] 실측으로 4.4 를 기각하고 4.0 으로 되돌린 기록이 있다. 그 실험은
        #   ★v=2.8 동작점에서 표값 자체를 올린★ 것이고, 그때 표의 최대는 4.0 이하였다.
        #   그런데 kasa 는 상한이 4.42 m/s(5펄스)로 올라가면서 표의 최상단이 6.40 이 되었다
        #   — 4.0 고정을 그대로 두면 v≳2.75 부터 ★표가 조용히 무효화★ 되고 ω_n 이 실측
        #   불안정 임계(≈1.2)를 넘어간다(LFD_TABLE 의 [2026-08-05] 항목 참고).
        #   그게 바로 이 파일 곳곳이 경고하는 "상한 이상인 캡이 조용히 죽는" 함정이다.
        #   ★기존 저속 거동은 하나도 안 바뀐다★ — v ≤ 2.75 구간의 표값은 전부 4.0 이하라
        #   클램프에 닿지 않았다. 이 변경은 '표보다 낮은 천장'을 걷어내는 것뿐이다.
        #   max(4.0, ...) 으로 두어 표를 저속용으로 줄여도 천장이 4.0 밑으로 내려가지 않게 한다.
        self.max_lfd   = max(4.0, LFD_TABLE[-1][1])
    ...
    def _apply_speed_scaling(self):
        ...
        # ★[2026-08-05] min_speed_ms 파라미터가 0 보다 크면 그 값을 쓴다★
        #   기본(0)은 종전과 동일한 '상한 비례' 산출. 위 declare_parameter 주석 참고.
        override = getattr(self, '_min_speed_override', 0.0)
        if override > 0.0:
            self.min_speed_ms = min(mx, max(MIN_SPEED_FLOOR, override))
        else:
    ...
    def _on_set_parameters(self, params):
        ...
        for p in params:
            ...
            elif p.name == "min_speed_ms":
                # ★[2026-08-05] 하한 override 런타임 조정★ 0 이면 상한 비례 산출로 되돌린다.
                #   상한을 5펄스로 올리면 하한이 2펄스로 따라 올라가므로, 저속 시험에서
                #   1펄스(0.9)로 되돌리고 싶을 때 쓴다.
                old = self.min_speed_ms
                self._min_speed_override = max(0.0, float(p.value))
                self._apply_speed_scaling()
                self.get_logger().warn(
                    f"[속도] min_speed_ms 전환: {old:.2f} → {self.min_speed_ms:.2f} m/s "
                    f"(≈{ku.ms_to_pulse(self.min_speed_ms)}펄스)"
                    + ("  [자동 = 상한 비례]" if self._min_speed_override <= 0.0 else ""))
