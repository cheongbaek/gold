...
pot    = 1.26·|δ| + 5.17·v_eff²·tan|δ|/L,  ±40 클램프  ← 부호는 − 좌 / + 우
         v_eff = min(v, 4펄스)   ★[2026-09-13] 아래 4.1b★
```

**LFD = v·√2 / ω_n**, **하한 4.04 m** [2026-09-13 저녁 갱신] / 상한 **11.3 m**.
`ω_n` 은 **[2026-09-13] 속도 스케줄**이다 — 4펄스 이하 `0.97`(`LFD_OMEGA_N`,
종전 그대로), 7펄스 이상 `0.78`(`LFD_OMEGA_N_FAST`), 사이는 펄스 선형보간.
표가 아니라 설계식을 옮긴 이유는 `drive_pulse` 를 바꾸면 자동으로 따라오게 하려고.

| 펄스 | v [m/s] | ω_n | **LFD** | 곡률캡 K | 비고 |
|---|---|---|---|---|---|
| 4 | 3.54 | 0.970 | **5.16 m** | 1.50 | ★종전과 완전히 같다★ |
| 5 | 4.42 | 0.907 | 6.89 m | 1.73 | 보간 |
| 6 | 5.30 | 0.843 | 8.89 m | 1.97 | 보간 |
...
`lookahead_m` 의 `v = self.drive_pulse * MS_PER_PULSE` 는 `__init__` 에서 한 번
정해진 값이다. 그런데 이 노드가 실제로 내는 속도는 **2 ~ 10펄스**다:

| 용도 | 펄스 |
|---|---|
| 순항 `drive_pulse` | 7 (상한 10) |
| 코너 하한 `corner_min_pulse` | `int(dp/2)` → 7펄스면 **3** |
| **종점 유지 · 라이다 인계 · 재수렴** | **2** |
| 헤딩 초기화 / 재출발 킥 | 3 |

그래서 저속 구간에서 LFD 가 속도에 비해 과도하게 길다:

| 설정 | 직선 LFD | 2펄스로 달릴 때 실효 ω_n | 설계 대비 |
|---|---|---|---|
| 4펄스 | 5.16 m | 0.48 | 50% |
| **7펄스** | **11.22 m** | **0.22** | **29%** |

#### ★그 결과 재수렴(REJOIN)이 원리적으로 못 끝난다★

| 설정 | 직선 LFD | 불감대 잔류 CTE | `REJOIN_DONE_CTE_M` 0.5 m |
|---|---|---|---|
| 4펄스 | 5.16 m | 0.26 m | OK |
| **7펄스** | 11.22 m | **1.24 m** | **★못 붙는다★** |
| 10펄스 | 11.30 m | 1.26 m | ★못 붙는다★ |

재수렴 중에는 **CTE 적분이 금지**되므로(4.6절) 불감대를 넘길 수단이 없고,
`REJOIN_TIMEOUT_S`(12 s)를 넘기면 **`S_DRIVE_DONE` = 정지 + 리니어 2단**이다 —
**L 구간을 잘 빠져나와 경로 위에서 멀쩡히 달리다 12초 뒤에 선다.**
문턱 0.5 m 를 넘기려면 LFD 가 **7.13 m 아래**여야 한다.

> **고칠 방향 셋** (실차 로그로 먼저 확인한 뒤 정하기로 했다):
> ① LFD 를 **직전 틱의 지령 펄스** 기준으로 계산 — 근본적이지만 LFD 전체 거동이
>    바뀐다. `lookahead_m` 의 "측정속도가 아니라 지령속도를 쓴다" 원칙은 지켜진다.
> ② **재수렴 구간만** `REJOIN_PULSE_MAX`(2펄스) 기준으로 — 종점 캡과 같은 패턴,
>    변경 범위 최소. **현재 권고안.**
> ③ `REJOIN_DONE_CTE_M` 을 올린다 — 간단하지만 증상만 가린다.
>
> ⚠️ **하한 4.04 m 는 이 문제와 무관하다** — 그 잔류는 0.16 m 로 문턱의 32% 다.
> 문제는 **상한 쪽이 저속 구간까지 적용되는 것**이다.
>
...
**증상** — 자율주행이 실 차선 기준으로 오른쪽에 치우쳐 달렸다. 9/13 네 주행 전부
CTE 부호가 **73~80% 오른쪽**, 사용자가 지적한 세 구간에서는 **85~100%**(한 구간은
단 한 틱도 왼쪽에 있지 않았다).

**원인** — 요레이트가 거의 0 인 표본(직진)만 2824개 모으면 실측 pot 중앙값이
**정확히 −3.00°** 다(네 주행 전부). **이 차는 pot 을 −3° 로 물고 있어야 직진한다.**
도로휠로 −1.9°, 보정 없이 100 m 를 가면 **3.4 m** 가 밀린다.

> ⚠️ **로그로는 두 가지를 못 가른다** — ⓐ pot 영점 편의와 ⓑ 차량 쏠림
> (얼라인먼트·공기압·좌우 인휠 출력차)이 **둘 다 실측 pot −3° 로 똑같이 나타난다.**
...
실측 pot(`/steer_angle_measured`)과 투영 요레이트, GPS 속도로 선형회귀하면
**기울기가 전달비 G, 절편이 트림**이다. 직진 구간만 골라낼 필요가 없다 —
코너 표본이 오히려 기울기를 잡아 준다.

**13개 로그 검증**

| 묶음 | 전달비 G | **trim** | 잔차 RMS |
|---|---|---|---|
| 9/12 오전 | 1.09~1.31 | −4.6 ~ −4.9° | 1.6~1.8° |
| 9/12 낮 | 1.26~1.34 | −7.5 ~ −7.9° | 1.3~1.6° |
| **9/12 늦음~9/13 (6개)** | 1.40~1.43 | **−2.63 ~ −2.90°** | 1.3~1.8° |

날짜별로 −7.9 → −4.9 → −2.9 로 계단처럼 바뀌었다 — **코드가 아니라 하드웨어 상태가
바뀐 것**이고, 추정이 그것을 따라갔다는 뜻이다. 수렴은 **5초(100표본)에 −2.82°,
10초에 −2.89°**, 표준오차는 표본 300개에 0.083°.

**위험 넷 다 작다 (실측)** — 자이로 바이어스(−0.006~+0.071 °/s)의 영향 **0.01° 미만** ·
한쪽으로만 도는 코스에서 좌/우 표본을 나눠 회귀해도 차이 0.38~0.65° ·
불감대 잡음은 잔차 RMS 1.3~1.8°(불감대 1.78° 수준) · 속도 오차는 **절편에 거의 무영향**.

#### ★CTE 적분과 중복이 아니다 — 역할 분담이다★

| 로그 | 적분 상태 | | 적분 기여(pot) | CTE 평균 | −(우) |
|---|---|---|---|---|---|
| 9/12 142144 | **−8.000** | **★포화★** | −3.06° | −0.242 m | **97.1%** |
| 9/12 120439 | **−8.000** | **★포화★** | −3.38° | −0.331 m | 98.3% |
| 9/13 144027 | −0.692 | | −0.43° | −0.033 m | 73.9% |

trim 이 −7.9° 이던 날은 **CTE 적분이 `CTE_I_CLAMP`(8.0)에 포화**해 있었고, 못 지운
나머지가 그대로 CTE 편향으로 남았다. **트림이 상수분을 가져가면 적분이 본래 일
(노면 경사·바람 같은 잔여 외란)을 할 여유를 되찾는다.**

#### ★★ 되먹임 루프가 아예 성립하지 않는다 — 이것이 안전의 근거다 ★★

`pot_meas` 는 **트림이 이미 포함된 실측값**이므로, 트림을 적용해도 절편은 제자리다:
...
목표다(`target = obs`). **적용값이 스스로를 키우는 경로가 없어 폭주가 불가능하다.**

> ⚠️ 처음에는 `target = 적용값 + 절편` 으로 두고 "자기소멸" 이라 보았는데 **틀렸다** —
> 로그 재생 검증에서 적용값이 상한까지 폭주해 그 자리에서 드러났다. 설계 오류를
> 시뮬레이션이 잡은 사례다.

**구현** — `driving.py` 의 `update_trim_estimate()`(관측, 매 틱 O(1) 누적회귀 +
지수망각) / `apply_trim()`(슬루 적용) / `_with_trim()`(지령에 더하고 재클램프).
`/steer_angle_measured` 를 **제어가 처음으로 구독한다**(종전에는 `record` 만 봤다).

| 상수 | 값 | 뜻 |
|---|---|---|
| `TRIM_MIN_N` | 300 | 이만큼 모이기 전에는 **적용하지 않는다**(≈15 s) |
| `TRIM_FORGET` | 0.999 | 유효표본 ≈1000(≈50 s 창) — 적용 후 조건 변화를 따라간다 |
| `TRIM_SLEW_DPS` | 0.3 | 3° 를 10초에 간다 |
| **`TRIM_MAX_POT_DEG`** | **5.0** | 이 밖이면 **자르고 경고** — 그만큼 필요하면 캘리브로 고칠 일이다 |
| `TRIM_LAG_TICKS` | 2 | pot → 요레이트 0.10 s (실측 상관 최대) |

**주행마다 0 에서 다시 추정한다**(사용자 결정) — `enter(S_DRIVE_HEADING)`/`S_IDLE`
에서만 리셋하고, 주행 중 상태 전이(L 구간 왕복 등)에서는 지우지 않는다.

**로그 재생 검증** (실주행 CSV 를 그대로 먹여 추정기를 돌렸다)

```
144027  회귀 실측 −2.90 → 수렴 −2.79   (30s −2.93 / 60s −2.95)   경고 0
134210  회귀 실측 −2.73 → 수렴 −2.72                              경고 0
141338  회귀 실측 −7.64 → ★상한 −5.00 에서 잘리고 경고★ (설계대로)
```

> ⚠️ **이것이 영점 캘리브를 대신하지 않는다.** trim −2.9° 는 조향 권한의 7%(±40 중)
> 를 상시 한쪽으로 쓰는 것이라 **반대쪽 최대 조향이 그만큼 줄어든다.** 자동 트림은
> '남은 오차를 지우는 장치' 이고, 하드웨어 영점은 그 값 자체를 0 에 가깝게 만드는
> 일이다. `BOARD_B.md` 3절의 `a` 캘리브와 **둘 다** 해야 한다.

### 4.1c ★LFD 하한 2.3 → 4.04 m — 2.3 은 1/5카 값이었다★ [2026-09-13 저녁]

`LFD_MIN_M = 2.3` 은 주석이 이미 밝히고 있듯 **구 white 의 `min_lfd`**, 즉 1/5카
실측값이다. 금색차에서 그 값은 **포화 영역 한복판**이다:

```
LFD 2.30 m → 순수추종 최대각 atan(2L/LFD) = ★47.4°★ > 도로휠 상한 31.7°
```

포화한 제어기는 계전기(bang-bang)가 되고 반드시 떤다 — 같은 날 braketest 에서
정확히 그 원리로 조향 부호반전 129.9회/분이 났다(5.1절).

**4.04 = 2 × 최소회전반경(2.02 m) 이 특별한 이유는 하나다:**

```
atan(2L/4.04) = ★31.7° = 도로휠 상한과 정확히 일치★
```

즉 **조향 권한을 하나도 잃지 않으면서 포화만 사라지는 유일한 점**이다. 더 올리면
그때부터는 권한을 잃는다 — 4.5 m 에서 2.7°, 5.0 m 에서 5.2°.

> **숫자가 아니라 식으로 두었다** — `LFD_MIN_M = 2·L / tan(STEER_ROAD_MAX_DEG)`.
> 휠베이스나 조향비를 바꾸면 반드시 함께 움직여야 하는 값이고(7절 '단일 소유자'),
> 이렇게 두면 경계에서 부동소수 때문에 '포화 가능'으로 미끄러지지도 않는다.

**무엇이 나아지나** — 코너에서 `CORNER_MIN_PULSE` 하한이 ω_n 결속 (b2) 을 이기는
최악 조합의 실효 ω_n (과조향 1.44 포함, 발산임계 2.3):

| | 종전 2.30 m | **신규 4.04 m** |
|---|---|---|
| 4펄스 (코너 2펄스) | 1.57 | **0.89** |
| **7펄스 (코너 3펄스)** | **2.35 ★초과★** | **1.34** |
| 10펄스 (코너 5펄스) | **3.91 ★초과★** | 2.23 |

실측 근거 — 2026-09-12 7펄스 로그(`161745`)의 LFD 는 **출발 직후 3.86~4.4 m** 였고
전 구간의 **13.6%** 가 4.05 m 미만이었다. 그 구간이 포화 가능 영역이었다.

> ⚠️ **대가는 코너컷이다.** 곡률캡 `√(K·R)` 이 이 하한에 눌리는 구간에서
> `LFD²/(8R)` : **R=5 m → 0.41 m, R=10 m → 0.21 m**. 눌리기 시작하는 반경은
> 4펄스(K=1.5) `R<10.9 m`, 7펄스(K=2.2) `R<7.5 m` 다. CTE 적분이 지우는 범위 안이고
> 이탈 문턱 2.0 m 의 20% 지만, **실차에서 코너 진입 궤적을 반드시 볼 것.**

> ### ⚠️ 함께 고친 것 — 종점 접근 캡이 하한에 먹히던 문제
>
> `lookahead_m` 의 마지막이 `return max(self.lfd_min, lfd)` 라, 하한이 4.04 로
> 올라가면 **종점 6.1 m 이내에서 종점 캡(`LFD_GOAL_*`)이 통째로 무력화**된다.
> 그러면 그 절의 첫 문단이 경고한 그대로가 된다 — *목표점이 마지막 WP 에 붙박이가
> 되고, 가까워질수록 α 가 커져 도착 직전에 조향이 크게 흔들린다.*
>
> → **종점 캡이 걸릴 때는 하한이 `LFD_GOAL_MIN`(2.2 m)** 이 된다. 안전한 이유는
> 그 구간이 `GOAL_HOLD_PULSE`(2펄스 = 1.77 m/s)까지 내려온 저속이기 때문이다:
> LFD 2.2 m 에서도 실효 ω_n 이 1.64(과조향 포함)로 임계 2.3 아래다.
> **하한의 근거인 '고속 코너의 포화'가 여기엔 없다.**
>
> ```
> 종점까지 10.0 m → LFD 5.80 (하한)    6.0 m → 4.00 ★캡★    1.5 m → 2.20 ★캡★
> ```
...
### 4.1b ★고속 조향 발산 — 언더스티어 항이 실측으로 성립하지 않는다★ [2026-09-13]

**증상** — 2026-09-12 7펄스 주행(`161745`·`145723`)이 **출발 직후 가속 구간부터**
조향이 **주기 1.7 s 로 발산**했다. 헤딩오차 진폭 3° → 5.6 → 6.9 → 8.0 → 13.8 → 23°,
매 주기 **1.4배**. 같은 경로 4펄스(`142144`)와 비교하면:

| 로그 | 기본속도 | 조향 부호반전 | 요레이트 std | max\|CTE\| |
|---|---|---|---|---|
| `142144` | 4펄스 | **0.6 회/분** | 3.48 °/s | 0.71 m |
| `145723` | 7펄스 | 20.7 회/분 | 7.31 °/s | 6.25 m |
| **`161745`** | 7펄스 | **22.8 회/분** | 6.80 °/s | 0.98 m |

> ⚠️ **직전 `curve_cap` 계단 수정(680b1b3)으로는 해결되지 않았다.** `161745` 는
> **그 커밋 뒤에 달린 로그**인데 여전히 22.8회/분이다. CTE 는 6.25 → 0.98 m 로
> 크게 좋아졌지만(그 수정은 그 수정대로 옳았다) **조향 떨림 자체는 그대로였다** —
> 원인이 다른 곳에 있었다.

**원인** — `record` CSV 의 **실측 pot**(`/steer_angle_measured`)과 **투영 요레이트**로
전달비 `G = |pot| / |atan(r·L/v)|` 를 직접 회귀했다(4개 로그 **21340표본**,
pot→요레이트 지연 0.10 s 에서 상관 최대 `r`=0.95):

| \|δ\| | 2-3 m/s | 3-4 m/s | 4-5 m/s | ← **실측** / **모델** → | 2-3 | 3-4 | 4-5 |
|---|---|---|---|---|---|---|---|
...
**★큰 각에서 G 가 1.42~1.52 로 수렴하고 속도를 따라 늘지 않는다★** — 이 차의 참
전달비는 **pot ≈ 1.5 × 도로휠각**이고, `5.17·v²` 언더스티어 항은 **실측되지 않는다.**
작은 각에서 G 가 커지는 것은 물리가 아니라 **조향 불감대**다(pot 은 움직였는데
요레이트가 아직 안 난 표본). 물리 전달비라면 각도와 무관해야 하는데 단조 감소하는
것이 그 증거다.

> ⚠️ **5.1절 braketest 의 '회귀 실측 5.89' 는 고속 직선의 1~2° 표본**이라 위 표
> 첫 행과 같은 불감대 아티팩트다. 그쪽은 조향 권한을 `ay_max` 로 따로 묶고 있어
> 당장 위험하지 않지만, **같은 회귀를 큰 각으로 다시 할 것.**

**그래서 무슨 일이 벌어졌나** — 순수추종이 δ=5° 를 원하면

| 속도 | 코드가 내는 pot | 차가 실제로 만드는 도로휠각 | 루프이득 |
|---|---|---|---|
| 4펄스 (3.54 m/s) | 2.15·δ | 2.15/1.50 | **1.44배** (과다하지만 PM 이 버틴다) |
| **7펄스 (6.19 m/s)** | **3.86·δ** | 3.86/1.50 | **★2.57배★** |

실효 `ω_n = (v√2/LFD) × 과조향배율` 로 보면 — **4펄스 1.40**(임계 2.3 아래) /
**7펄스 2.61**(임계 초과 = 발산) / 진동 구간(t=15~21 s, 곡률캡 LFD 7.0 m, v 5.3 m/s)
**2.35 — 임계 초과.** ★관측된 발산이 계산으로 그대로 재현되고, 4펄스만 멀쩡했던
이유까지 설명된다.★

**고친 방법 — 계수를 낮추지 않고 '속도를 묶는다'**

계수를 0 으로 지우거나 깎으면 **4펄스 이하의 검증된 거동까지 같이 약해진다**
(`142144` 는 max|CTE| 0.71 m 로 잘 돌았다 — 그 이득을 건드릴 이유가 없다).
**속도만 4펄스에서 클램프한다**(`UNDERSTEER_V_CLAMP_PULSE = 4`):

- `v ≤ 3.536 m/s` : **식이 한 글자도 바뀌지 않는다** — 중저속 거동 완전 동일
- `v > 3.536 m/s` : G 가 2.15 에 멈춘다 → 7펄스 과조향 2.57 → **1.44배**
  = **4펄스와 같은 실효 루프이득**

여기에 4.1 의 **ω_n 속도 스케줄**과 **곡률캡 K 스케줄**(1.5 → 2.2)을 더하면:

```
진동 구간 재현 (요구곡률 2.19°, v 5.3 m/s)
  종전 : LFD 7.00 m  과조향 2.19배  실효ω_n 2.35   ★발산임계 2.3 초과★
  신규 : LFD 8.48 m  과조향 1.44배  실효ω_n 1.28   여유 80%
```

> **곡률캡 K 도 함께 푼 이유** — ω_n 만 낮춰 `lfd_speed` 를 11.2 m 로 늘려도,
> 요구곡률이 조금만 있으면 곡률캡이 먼저 눌러 '고속에서 더 멀리 본다'가 성립하지
> 않는다. 실제로 그 진동 구간의 LFD 를 정한 것은 `lfd_speed` 9.02 m 가 아니라
> **곡률캡 7.0 m** 였다. 대가는 코너컷 오차뿐이다(`K/8` : 0.19 → **0.275 m**).
> **코너에서는 `corner_speed` 가 속도를 내리고, 그러면 K 도 1.5 로 돌아와 조향
> 권한이 복구된다** — 즉 이 스케줄이 실제로 듣는 곳은 **고속을 유지하는 완만한
> 구간**뿐이다.

> ⚠️ **실차에서 먼저 볼 것** ① 조향 부호반전 [회/분] — 4펄스의 0.6 에 가까워지는가
> ② `mean|CTE|` — LFD 가 길어지면 조향 불감대가 만드는 잔류 편향도 함께 늘어난다.
> ★[2026-09-13 정정] 이 값은 LFD 에 비례가 아니라 **제곱**으로 커진다★ —
> `e = δ·d²/(2L)` (δ = 도로휠 불감대 1.41°). 검산 3.00 → 0.089 / 5.16 → 0.262 /
> **11.22 → 1.24 m**. 여기 처음 적었던 "11.2 m → 0.57 m" 는 비례로 잘못 계산한 값이다.
> 일반 주행에서는 CTE 적분이 지운다(`CTE_I_MAX_DEG 2.5°` 가 불감대 1.41° 의 1.8배).
> **다만 재수렴 구간은 적분이 금지라 그렇지 않다 — 4.1b-2 절.**
> ③ 코너 진입 궤적(코너컷).
>
...
> ~~**`braketest.py` 에는 이 코드를 넣지 않았다.**~~ **[2026-09-10 정정] 넣었다.**
> 처음에는 "직선 전용이라 요 적분의 누적오차가 문제 되는 구간이 없다" 고 봤는데,
> 그 노드는 **요레이트를 적분이 아니라 ★댐핑★ 으로 쓴다** — 축이 58° 틀어져 있으면
...
### 5.1 제동검차 — 목표 좌표 한 점을 향한 직선 질주 [2026-09-09 신설 / **2026-09-12 전면 개편**]

```bash
ros2 launch white1 braketest.launch.py                       # ★상단 상수의 목표로★
ros2 launch white1 braketest.launch.py goal_lat:=37.1 goal_lon:=127.1
...
**이 노드의 존재의의는 제동검차 통과(30 km/h 이상에서 급제동) 하나다.** 그래서
저속 예행 없이 **처음부터 고속으로 달리고**, 조향은 '고속 직선에서 안 흔들리는 것'
하나에만 맞춰져 있다.

> ### ★[2026-09-12] 경로 CSV 추종을 버렸다 — 기준은 '점'이 아니라 '선' 이다★
>
> **목표 좌표의 단일 소유자는 `braketest.py` 상단의 `GOAL_LAT` / `GOAL_LON` 이다**
> (런치 인자 `goal_lat`/`goal_lon` 은 그것을 덮는 임시 수단, `route:=` 는 그 CSV 의
> **마지막 점**을 목표로 빌려 오는 폴백일 뿐 — **경로를 추종하지 않는다**).
>
> ```
> S = 출발 순간 차의 위치(RTK)      G = 목표 좌표      û = (G−S)/|G−S|
> s = (P−S)·û  진행거리   e = (P−S)·n̂  횡오차(+ 왼쪽)   ψ = 헤딩 − 선방위
> ```
>
> - **기준에 잡음이 없다** — 두 좌표로 만든 방위는 주행 내내 변하지 않는다.
>   종전의 사행 재료(목표 계단·최근접점 점프·포인터 도약)가 통째로 사라진다.
> - **`e(0)=0`, `ψ(0)≈0` 이 설계상 보장된다** — S 가 차 자신의 출발점이고, 사람이
>   차를 목표 좌표를 바라보게 세우기 때문이다. 그 전제는 **출발 1초 뒤 GPS 코스로
>   검산**하고, 8° 넘게 틀어져 있으면 **그 자리에서 2단을 물고 접는다.**
> - **`e` 는 RTK 로 직접 관측되고 `ψ` 는 추정값이다** — 되먹임의 주력을 위치에 둔다.

| 트리거 | 무엇 |
|---|---|
| **① 라이다 장애물** | `cone_lidar_node` 의 `stop_signal` → `pedal_drive_node` → arduino |
| **② ★목표 좌표 도달★ (FAIL-SAFE)** | **`s ≥ D` 인 순간 즉시 리니어 2단.** 반경이 아니라 **진행거리 투영**이라 한 틱에 0.44 m 를 가도 못 놓친다 |

> **[2026-09-09] 정지 트리거가 `terrain` `'S'` 에서 라이다로 바뀌었다.**
> `terrain` 열은 **이제 보지 않는다.** (2026-09-12 부터는 CSV 자체를 안 읽는다.)
> (`driving.py` 의 `'S'` 일시정지는 그대로다 — 그쪽과 무관한 변경이다.)

#### ★조향 — '멀리 본다' 의 정확한 뜻 : 먼 점은 방향을 주고, 이득은 프리뷰가 준다★

**끝점을 직접 겨누면 이득이 0 이 된다**(2026-09-09 에 실제로 겪었다). 순수추종식
`δ = atan(2L·sinα/d)` 은 d 가 분모라, d=70 m 면 횡오차 1 m 가 pot 0.06° = B보드
조향 불감대(**pot 1.78°**)의 **28분의 1** 이다 — 핸들이 한 번도 안 움직인다.

```
e_p = e + L_p·sin ψ                 L_p = t_preview × v   ★정속 프리뷰★
δ   = atan(2·L·e_p / L_p²)          (= 2L/L_p²·e + 2L/L_p·ψ)
```

| t_preview | L_p (8.84 m/s) | ω_n | 헤딩루프 PM | 위치이득 | 불감대를 넘기는 e |
|---|---|---|---|---|---|
| 1.2 s | 10.6 m | 1.18 | +37° | 8.8 pot°/m | 0.20 m |
| **1.5 s (기본)** | **13.3 m** | **0.94** | **+48°** | **5.6 pot°/m** | **0.32 m** |
| 2.0 s | 17.7 m | 0.71 | +58° | 3.2 pot°/m | 0.56 m |

`ω_n 0.94` 는 `driving.py` 의 `LFD_OMEGA_N 0.97` 과 같은 자리다 — **이 차에서 검증된
유일한 안정도 기준.** 마지막 열이 **정상상태 잔류 횡오차**다(불감대가 성능 하한을
정한다). 차로가 더 좁으면 `t_preview:=1.2` 가 첫 손잡이.

#### ★종전 사행의 원인 — 헤딩 이득 8.8배 과다 (기록)★

종전 식은 `δ = 1.0·ψ + atan(0.4·e/v)` 였고, 실효 프리뷰 22 m 의 순수추종 이득은
`2L/L_p = 0.113` 인데 **1.0** 을 썼다. 헤딩루프 교차주파수 `ω_c = K_ψ·v/L` 로 보면
**2.21 rad/s**(위상여유 +20°)이고, 지연 0.55 s 루프의 위상 −180° 점은
**2.86 rad/s = 주기 2.2 s** 다 — 실측 진동이 **주기 2.4 s, 진폭 ±3.8°, cmd 는 −5° 에
붙은 채**였다. **조향이 흔들린 게 아니라 이득이 교차주파수를 지연 영역까지 밀었다.**

#### ★전달비(pot/도로휠) — 속도 클램프가 루프이득을 3.2배 깎고 있었다★

| 도로휠 1° 기준 | pot |
|---|---|
| 종전 코드 (언더스티어 항의 v 를 4펄스로 클램프) | 2.16 |
| 모델을 실제 속도로 (v = 8.84) | 6.90 |
| **로그 회귀 실측 (6~9 m/s, 468표본)** | **5.89** |

즉 이 속도에서는 모델이 맞았고 클램프가 **지령의 31% 만 꺾이게** 만들고 있었다.
...
- `ay_max`(기본 2.0 m/s²)에서 pot 상한을 유도한다(8.84 m/s 에서 ≈12.6°).
  종전 `5.0°` 는 실측 전달비로 환산하면 도로휠 0.72°(횡가속 0.79)뿐이라 **항상
  포화**했고, 포화가 곧 계전기 진동이었다.
- **전달비를 주행 중에 잰다** — `/steer_angle_measured`(실측 pot)와 투영 요레이트로
  `G = |pot| / |atan(r·L/v)|`. **조이는 쪽으로만** 쓴다(모델보다 예민한 것으로
  드러나면 권한을 줄이고, 둔하면 아무것도 안 한다). **결과 표에 찍히므로 다음 시험의
  튜닝 근거가 이 한 줄에서 확정된다.**
- 재기 전까지는 `POT_START_MAX_DEG 6.0` 으로 묶는다 — **모르면 낮게 본다.**
- **중단은 필터된 횡가속도로 본다** — 순시 요레이트로는 **계전기 진동을 못 잡는다**
  (반주기마다 부호가 뒤집혀 '연속 초과'가 성립하지 않는다. 책상 검증에서 횡가속
  9.2 m/s² 가 나는 동안 순시 판정은 한 번도 안 걸렸다).
...
>
> **목표 좌표(또는 장애물) 뒤로 최소 30 m 가 비어 있어야 한다.**
> **목표 좌표는 "여기서 서라" 가 아니라 "여기서 밟기 시작하라" 다** — 이 노드는
> 그 점을 지나 **멈출 때까지 계속 달린다.** 앞에는 가속 구간이 더 붙는다.
> 목표까지의 거리가 20~400 m 밖이면 **출발하지 않는다**(좌표 오타 방지).
>
...
| **LFD 하한 2.3 → 4.04 m (4.1c) [2026-09-13 저녁]** | `driving.py LFD_MIN_M`(유도식) · `lookahead_m` 종점 캡 분리 · `one_launch.py lfd_min_m` |
| **조향 트림 자동 추정 (4.1d) [2026-09-13 저녁]** | `driving.py update_trim_estimate` · `apply_trim` · `_with_trim` · `cb_steer_measured` · 상수절 `TRIM_*` |
| **mp4 세그먼트 분할 (5.2) [2026-09-13]** | `nxde/video.py SEGMENT_S` · `_roll_segment` · SIGINT 핸들러 · `kill.py RECORDER_*` · `camera_launch.py tl_video_segment_s` |
