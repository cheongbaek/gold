...
| **윤거(track width)** | **1.08 m** (1080 mm) — ★2026-09-09 정정★ | `../mad-code/CLAUDE.md` 1절 (하드웨어 정본) |
...
> **1080 mm** 이고 정본은 **`../mad-code/CLAUDE.md` 1절**
> (*"차량폭(윤거) : 1080 mm"*, 2026-09-09 기재)이다.
...
> | `mppi .../vehicle_model.hpp:32,98` | 1.10 (정본 +20 mm) | 충돌 판정 반폭 `half_w` |
> | `mppi .../node.cpp:434,452` | 1.10 (정본 +20 mm) | `robot_half_width = track/2 + 0.08 = 0.63`, ego_clear 반폭 |
> | `mppi/config/params.yaml:84` | 1.10 (정본 +20 mm) | 위 둘의 입력 |
> | `white1/hud.py:218` | 1.10 (정본 +20 mm) | 화면 차체 그림 폭(표시 전용) |
...
>
> **② `mppi` 라바콘 회피 : 사실상 맞다.** 1.10 기준 반폭 0.55(정본이면 0.54),
> 코스트맵 lethal 반경 0.63 m. **20 mm 차이는 반폭 10 mm** 라 통과 판정에 영향이
> 없고, 넓게 보는 쪽이라 방향도 안전하다.
>
...
**[2026-09-09] 1·2단은 이제 "유지"된다** — `LIN_HOLD` 가 목표 밑으로 내려간 만큼을
PWM 100 으로 되민다(위 불변식 0). 그래서 `/brake_pot` 이 지시 중에 목표 밑에 오래
머무르면 **기구가 막힌 것**이다(B보드 디버그에 `# LIN HOLD TIMEOUT`).
...
**A5 ≥ 350** 하나뿐(`kasa_0909_B.ino:323`, 0821 초판 400 → 실차에서 350).
브레이크 단계도 주행모드도 E-stop 도 보지 않는다.

> ### ★★ 리니어에 관한 불변식 ★★
> 0. **[2026-09-09 / `kasa_0909_B.ino`] 1·2단 동안에는 B보드가 스스로 되민다.**
>    A5 가 목표(600/850)보다 `BRAKE_SAG`(10) 만큼 내려가면 **PWM 100 으로 천천히**
>    목표까지 따라간다(`LIN_HOLD`). **올라갈 때는 움직이지 않는다.**
>    → **사람이 페달을 밟고 있으면 리니어가 거의 안 움직이는데, 그 상태에서 발을
>    떼면 제동력이 하나도 안 걸리던 것**을 고친 것이다. 아래 1번의 "시킨 대로만"
>    은 **단계 지시에 대해서만** 참이다 — 그 단계를 *지키는* 일은 보드가 한다.
...
| | A보드 (`kasa_0904_A.ino`) | B보드 (`kasa_0909_B.ino`) |
|---|---|---|
...
`BRAKE_HOME_MS = 1000 ms`** 돌린다(`kasa_0909_B.ino:297`). 게다가 arduino 에
