...
//
// 정지 관측 → 좌/우 라바콘 누적 → 복도 헤딩 잠금 → ★1펄스로 차 코를 맞춘 뒤★
// 조향을 0에 고정하고 순항 직진. 맞춘 뒤에는 헤딩 오차로 조향을 다시 넣지
// 않는다 (고속에서 주행 내내 헤딩홀드하면 IMU 잡음에 출렁인다).
// 주행 중 콘을 다시 붙잡지 않는다 (v5 EKF 옆길 이탈의 원인).
//
...
bool orientationValid(const sensor_msgs::msg::Imu& msg) {
  ...
  if (!(n2 > 0.25 && n2 < 4.0)) return false;
...
public:
  DriveLidarNode() : Node("drive_lidar_node") {
    ...
    // ★기본 7펄스 = 6.188 m/s ≈ 22.3 km/h★
    //   금색차의 속도 지령은 ★정수 펄스★ 다. 여기 m/s 로 무엇을 넣든
    //   KasaActuator 가 반올림한다. kasa.max_pulse 가 이보다 작으면 잘린다.
    this->declare_parameter<double>("linear_speed",
                                    lidar::kasa::pulseToMs(7));
    ...
    // ★외장 iAHRS 쿼터니언 yaw★ true 면 자이로 적분을 쓰지 않는다 (드리프트↓).
    //   /ouster/imu 는 orientation 이 비어 있어 자동으로 자이로로 내려간다.
    this->declare_parameter<bool>("imu_use_orientation", true);
    ...
    // ── 헤딩 정렬 (1펄스) 후 순항 (조향 고정) ──────────────────────────
    //  잠금 직후 순항속도에서 k_heading 을 계속 넣으면 고속에서 출렁인다.
    //  차 코가 맞을 때까지 1펄스로만 조향하고, 맞으면 조향 0 + 순항.
    this->declare_parameter<double>("align_speed", lidar::kasa::pulseToMs(1));
    this->declare_parameter<double>("align_done_deg", 2.5);
    this->declare_parameter<double>("align_steer_done_deg", 2.0);
    this->declare_parameter<double>("align_hold_s", 0.4);
    this->declare_parameter<double>("align_timeout_s", 8.0);
    ...
    // ★양쪽 라바콘이 잡혀야 복도를 잠근다★ 한쪽만 있으면 코스 중심이
    //   initial_half_width 가정으로 밀린다. 금색차 라바콘 코스는 양옆이 기본.
    this->declare_parameter<bool>("require_dual_side", true);
    ...
    RCLCPP_INFO(this->get_logger(),
                "drive_lidar v6: survey %.1fs → 1펄스 헤딩 정렬 → 순항. "
                "lidar=%s cruise=%.2f m/s (%.1f km/h) align=%.2f m/s "
                "done=%.1f° hold=%.2fs L=%.2fm",
                survey_duration_, lidar_topic.c_str(), linear_speed_,
                linear_speed_ * 3.6, align_speed_, align_done_deg_,
                align_hold_s_, wheelbase_);
    RCLCPP_INFO(this->get_logger(),
                "헤딩 IMU = '%s'  imu_use_orientation=%s  dual_side=%s",
                imu_topic.c_str(),
                imu_use_orientation_ ? "true" : "false",
                require_dual_side_ ? "true" : "false");
  }
...
private:
  ...
  enum class Phase { Survey, Align, Drive, Failed };
  ...
  void loadParamsToMembers() {
    ...
    this->get_parameter("imu_use_orientation", imu_use_orientation_);
    this->get_parameter("require_dual_side", require_dual_side_);
    ...
    this->get_parameter("align_speed", align_speed_);
    this->get_parameter("align_done_deg", align_done_deg_);
    this->get_parameter("align_steer_done_deg", align_steer_done_deg_);
    this->get_parameter("align_hold_s", align_hold_s_);
    this->get_parameter("align_timeout_s", align_timeout_s_);
  ...
  rcl_interfaces::msg::SetParametersResult onParamChange(
    ...
    for (const auto& p : params) {
      ...
      else if (name == "imu_use_orientation")
        imu_use_orientation_ = p.as_bool();
      else if (name == "require_dual_side")
        require_dual_side_ = p.as_bool();
      else if (name == "align_speed")
        align_speed_ = p.as_double();
      else if (name == "align_done_deg")
        align_done_deg_ = p.as_double();
      else if (name == "align_steer_done_deg")
        align_steer_done_deg_ = p.as_double();
      else if (name == "align_hold_s")
        align_hold_s_ = p.as_double();
      else if (name == "align_timeout_s")
        align_timeout_s_ = p.as_double();
    }
  ...
  void imuCallback(const sensor_msgs::msg::Imu::ConstSharedPtr& msg) {
    ...
    if (use_ori) {
      yaw_abs_ = yawFromQuaternion(msg->orientation);
      has_yaw_abs_ = true;
    }

    if (phase_ == Phase::Align || phase_ == Phase::Drive) {
      if (use_ori && has_yaw0_) {
        // 외장 iAHRS 쿼터니언 — 자이로 적분보다 드리프트가 작다.
        psi_ = normalizeAngle(yaw_abs_ - yaw0_);
      } else if (now > 0.0 && last_imu_time_ > 0.0) {
  ...
  bool tryLockCorridor() {
    ...
    RCLCPP_INFO(this->get_logger(),
                ...
                (imu_use_orientation_ && has_yaw0_)
                    ? "AHRS 쿼터니언"
                    : "자이로 적분",
                (imu_use_orientation_ && has_yaw0_) ? "" : " (드리프트 큼)");
    if (imu_use_orientation_ && has_yaw0_) {
      RCLCPP_INFO(this->get_logger(),
                  "AHRS 헤딩 잠금 yaw0=%.1f deg — 외장 iAHRS 쿼터니언. "
                  "자이로 적분을 쓰지 않는다.",
                  yaw0_ * 180.0 / M_PI);
    }
    RCLCPP_INFO(this->get_logger(),
                "↕ 헤딩 정렬 시작 — 1펄스(%.2f m/s)로 차 코를 %.1f° 에 맞춘 뒤 "
                "조향 고정 · 순항 %.1f km/h",
                align_speed_, lock_psi_ * 180.0 / M_PI, linear_speed_ * 3.6);
    return true;
  }

  bool requireBothSides() const { return require_dual_side_; }
  ...
  void cloudCallback(
    ...
    // 쿼터니언 헤딩이 살아 있으면 자이로를 더하지 않는다 (이중적분 방지).
    if (!(imu_use_orientation_ && has_yaw0_)) {
    ...
    // Align: 1펄스로 차 코만 맞춘다. Drive: 조향 0 고정 + 순항.
    // 순항에서 헤딩홀드를 계속 넣으면 IMU 잡음에 고속 출렁인다.
    double angular = 0.0;
    double speed_cap = linear_speed_;
    const double now_s = this->now().seconds();
    if (phase_ == Phase::Align) {
      speed_cap = std::max(align_speed_, lidar::kasa::pulseToMs(1));
      angular = std::clamp(k_heading_ * e_psi, -max_angular_speed_,
                           max_angular_speed_);
      if (lock_use_cte_) {
        angular += lidar::LineEkf::stanleySteer(
            e_y, 0.0, std::max(speed_cap, 0.5), k_stanley_,
            max_angular_speed_, 0.0);
        angular = std::clamp(angular, -max_angular_speed_, max_angular_speed_);
      }
      if (align_t0_ <= 0.0) align_t0_ = now_s;
      const bool heading_ok =
          std::fabs(e_psi) * 180.0 / M_PI <= align_done_deg_;
      const bool steer_ok =
          std::fabs(angular) * 180.0 / M_PI <= align_steer_done_deg_;
      if (heading_ok && steer_ok) {
        if (align_ok_t0_ <= 0.0) align_ok_t0_ = now_s;
        if (now_s - align_ok_t0_ >= align_hold_s_) {
          phase_ = Phase::Drive;
          angular = 0.0;
          RCLCPP_INFO(this->get_logger(),
                      "▶ 헤딩 정렬 완료 (e=%.2f°) — 조향 0 고정, 순항 %.1f km/h",
                      e_psi * 180.0 / M_PI, linear_speed_ * 3.6);
        }
      } else {
        align_ok_t0_ = 0.0;
      }
      if (phase_ == Phase::Align && align_timeout_s_ > 0.0 &&
          now_s - align_t0_ >= align_timeout_s_) {
        phase_ = Phase::Drive;
        angular = 0.0;
        RCLCPP_WARN(this->get_logger(),
                    "⏱ 헤딩 정렬 시간초과 (e=%.2f°, %.1fs) — 조향 0 고정하고 순항",
                    e_psi * 180.0 / M_PI, align_timeout_s_);
      }
    }
    ...
    // 순항 가속은 바퀴가 거의 곧게 된 뒤에만. 타임아웃으로 Drive 에 들어와도
    // 조향이 남은 채 7펄스가 나가면 고속에서 한 번 더 출렁인다.
    if (phase_ == Phase::Drive &&
        std::fabs(prev_angular_z_) * 180.0 / M_PI > align_steer_done_deg_) {
      speed_cap = std::max(align_speed_, lidar::kasa::pulseToMs(1));
    }
    ...
    double desired_v = speed_cap * std::min(gate_scale, aeb_scale);
    {
    ...
    if (log_counter_++ % 20 == 0) {
      RCLCPP_INFO(this->get_logger(),
                  "[%s] psi=%.2f/%.2f deg ey=%.2f v=%.2f aeb=%d d=%.1f",
                  phase_ == Phase::Align ? "ALIGN" : "CRUISE",
  ...
  double align_speed_ = lidar::kasa::pulseToMs(1);
  double align_done_deg_ = 2.5;
  double align_steer_done_deg_ = 2.0;
  double align_hold_s_ = 0.4;
  ...
  bool locked_ = false;
  double align_t0_ = 0.0;
  double align_ok_t0_ = 0.0;
  ...
  double yaw_abs_ = 0.0, yaw0_ = 0.0;
  bool has_yaw_abs_ = false, has_yaw0_ = false;
