...
#   아두이노 펌웨어(★kasa_0904_A.ino / kasa_0909_B.ino★)도 무수정 전제다.
#
...
#                                               켠다(kasa_0909_B.ino BRAKELIGHT_ON_RAW).
...
#       kasa B보드 : − 좌 / + 우 (kasa_0909_B.ino angleToPot: −40 → 왼쪽 끝 raw)
...
#     0~255 PWM 이 아니다(kasa_0909_B.ino). 안 오면 0(놓음)으로 둔다.
...
#
#  5) 브레이크는 0~255 PWM 이 아니라 **단계 0/1/2** 다 (kasa_0909_B.ino).
...
#     ★그래도 stop_and_close 는 그대로 둔다★ — 워치독은 최후의 그물이고, 정상 종료가
#     차를 3초 굴리고 세울 이유가 없다. 둘은 대체가 아니라 이중화다.
#     ⚠️ ★KEEPALIVE_S 와 RX_TIMEOUT_MS 는 한 쌍이다★ 이쪽을 3초 이상으로 늘리면
#       정상 주행 중에 보드 워치독이 걸려 구동이 끊긴다. 늘리려면 펌웨어를 함께 고친다.
#     (0713~0904-3 까지는 타임아웃이 아예 없어서, 상위가 SIGKILL·터미널 종료·USB
#      단선으로 말을 멈추면 ★마지막 명령을 영원히 물고 차가 계속 갔다★.)
...
# ★값이 안 바뀌어도 이 간격으로는 한 번 재전송한다★ A·B 양쪽에 같이 적용된다
#   (on_tx_timer 의 두 분기). 값이 바뀌면 TX_PERIOD_S(0.05s) 판정에서 즉시 나가므로,
#   이것은 전송 주기가 아니라 ★줄 간격의 상한★ 이다.
#   ★[2026-09-04] 이 값은 A보드 워치독(RX_TIMEOUT_MS = 3000)과 한 쌍이다★
#     보드는 3초 무입력이면 스스로 정지한다. 1초 keepalive 는 그것을 ★세 번 연속
#     놓쳐야★ 발동하게 하는 여유다 — USB 재열거나 한두 번의 전송 실패로는 안 걸린다.
#     ⚠️ 이 값을 늘리려면 펌웨어의 RX_TIMEOUT_MS 도 함께 늘려야 한다(셋 배 이상).
#   ★1.0 보다 짧게 줄이지도 말 것★ B보드 handleLine 은 줄을 받을 때마다 steer_state 를
#     ST_ACTIVE 로 되돌린다. SETTLE_MS(500ms) 도달판정이 keepalive 사이에 성립해야
#     하므로 1.0s 는 그 두 배로 잡은 값이다(위 '전송 정책' 절).
KEEPALIVE_S   = 1.0
...
#  펌웨어 : kasa_0904_A.ino [0904-3] / kasa_0909_B.ino [0904-2].
...
# ★E-STOP 중에는 B보드가 "STOP" 만 내보낸다★ (kasa_0909_B.ino sendOutput:
...
class Arduino(Node):
    ...
    def poll_port(self, which):
        ...
        # ★ 진단 줄('#'로 시작)은 걸러낸다 ★ kasa_0909_B.ino 의 DEBUG_B 와 조향 영점
    ...
    def parse_b(self, text):
        """"P,<조향각>,<A5원본>,<모드>" (kasa_0909_B.ino). 조향각 부호는 ★− 좌 / + 우★
    ...
    def _install_exit_handlers(self):
        ...
        핸들러가 없으면 파이썬 기본 동작으로 ★즉사★ 하므로
          · stop_and_close 가 돌지 않는다 → ★정지값이 보드에 안 나간다★
            (보드 워치독은 3초 뒤에야 걸린다 — 그 3초를 굴리지 않기 위한 처리다)
    ...
    def stop_and_close(self):
        ...
        ★ 보드 워치독보다 이쪽이 먼저다 ★ ([2026-09-04] kasa_0904_A.ino [0904-4] 로
        3초 무입력 정지가 생겼지만, 정상 종료가 차를 3초 굴릴 이유가 없다) 마지막 수신 명령을
