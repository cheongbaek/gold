...
★어안(fisheye) 왜곡보정에 대하여 — 지금 파이프라인에는 없다★
  white1/camera_launch.py 가 usb_cam 에 `camera_info_url` 을 ★일부러 주지 않는다★
  ("traffic_light 는 왜곡보정을 쓰지 않는다. 쓰지도 않을 캘리브 파일을 들이면 '맞는지
  아무도 모르는 값'이 하나 늘 뿐이다" — 그 파일 주석). 즉 `/image_raw` 는 렌즈 왜곡이
  남아 있는 상태이고, 그것이 곧 ★인지가 보는 화면★ 이다.
  → 그래서 이 노드도 왜곡보정을 하지 않는다. ★나중에 파이프라인에 보정이 들어가면
    /image_raw 자체가 보정된 그림이 되므로 이 노드는 고칠 것이 없다.★
    (보정을 이 노드에서만 따로 하면 '인지가 본 것'과 '기록된 것'이 갈린다 — 그러면
     영상으로 오검출을 따져도 근거가 되지 않는다. 그것이 여기서 안 하는 이유다.)
