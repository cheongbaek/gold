...
> **kasa_ws 쪽은 수정하지 않았다.** 아두이노 펌웨어는 **`kasa_0904_A.ino` / `kasa_0909_B.ino`**
...
nxde/tts.py         ★domichat 채팅방 → 음성★ <PC명>_TTS_M / <PC명>_TTS_W 두 방을
                    ...
                    파일 상단 [1] 파라미터 절이 단일 소유자다. ★GUI 가 없다★
                    (종전의 문장 입력 창은 제거됐다). 합성 edge-tts(망 필요),
                    재생은 sound.py 와 같은 외부 재생기라 ★안내 음성과 겹쳐
                    들린다★. 새 채팅이 오면 읽던 것을 끊고 그것을 읽는다.
...
- **E-stop** : 리니어 2단 체결과 해제(0단 복귀)는 **B보드 펌웨어가 스스로 한다**
  (`kasa_0909_B.ino`). ROS 가 브레이크를 지시할 필요가 없고, e-stop 중에는
