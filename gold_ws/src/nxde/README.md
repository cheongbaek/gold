...
> **kasa_ws 쪽은 수정하지 않았다.** 아두이노 펌웨어(`kasa_0804_A.ino` / `kasa_0804_B.ino`)도 무수정.
...
- **E-stop 확인시간이 500ms 다**(A·B보드 공통, `[0804-1]`/`[0804-5]`). 4.42 m/s 에서
  약 2.2m 진행 후 제동된다. **사람이 발로 밟는 것이 1차 수단인 전제**이며, 고속으로 올릴
  때는 `ESTOP_TRIGGER_CONFIRM_MS` 를 줄이거나 배선 노이즈를 먼저 해결할 것.
