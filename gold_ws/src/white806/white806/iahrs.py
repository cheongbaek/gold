#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""iahrs — iAHRS IMU 시리얼 드라이버 → ★/imu★ (Ubuntu 22.04 전용)
────────────────────────────────────────────────────────────────────────────────
★ [white806] 이 노드는 순수 드라이버다 ★
  6축 자이로·가속도 값을 받아 sensor_msgs/Imu 로 /imu 에 그대로 흘려보내는 것이
  전부다. GPS 와의 융합은 하지 않는다 — 그건 driving 이 직접 한다(구 gps_imu.py
  는 white806 에 없다). 이 파일은 one_launch.py 가 띄운다:
      ros2 launch white806 one_launch.py
  구 white 대비 바뀐 것은 ★토픽 이름 imu/data → /imu★ 하나뿐이고, 드라이버
  로직(재연결·포트 재탐색·공분산)은 손대지 않았다.

★ 1/5카 원본 대비 변경점은 '포트 재탐색' 하나뿐이다 ★
  ...
  → 재연결마다 white/ports.py 의 VID/PID 스캔을 한 번 더 돌려 경로를 다시 정한다.
...
class IahrsDriver(Node):
    ...
    def __init__(self):
        ...
        #   아두이노가 될 수 있다. udev 설정법은 nxde/README.md 8절 참고.
        #   런치 전 `ros2 run nxde check` 로 실제 경로를 먼저 확인할 수 있다.
        ...
        # GPS 로 이미 확정된 경로 — 재탐색에서 제외한다(그 포트를 열면 배타 open 충돌로
        #   해당 드라이버가 자기 포트를 못 잡는다). one_launch.py 가 넘겨준다.
        #   ※ 아두이노 경로는 넘기지 않아도 된다 — IMU_VIDPID(10c4:ea60) 와 아두이노
        #     VID(2341/1a86/2a03) 가 겹치지 않아 VID/PID 스캔이 애초에 아두이노를 안 고른다.
        ...
        # ★[white806] 토픽 이름을 /imu 로 바꿨다★ (구 white 는 imu/data)
        self._imu_pub_handler = self.create_publisher(Imu, "/imu", 10)
