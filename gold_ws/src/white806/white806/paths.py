...
우선순위 (양쪽 함수 공통):
  1) 노드 파라미터로 준 명시 경로
  2) 환경변수 (WHITE806_DATA_DIR / WHITE806_RECORD_DIR)
  3) ★소스 트리★ <...>/src/white806/{gps_data,ros2bag}
     — colcon 을 --symlink-install 로 빌드하면 이 파일이 소스를 가리키는
       심볼릭 링크라 realpath 로 소스 위치를 되찾을 수 있다.
  4) 못 찾으면 ~/white806/{gps_data,ros2bag}
...
def _package_root():
    """설치본이 아닌 '소스 트리의 white806 패키지 루트'. 못 찾으면 None."""
    here = os.path.dirname(os.path.realpath(__file__))   # .../src/white806/white806
    root = os.path.dirname(here)                         # .../src/white806
...
def _resolve(explicit, env_key, subdir):
    ...
    return os.path.expanduser(os.path.join('~/white806', subdir))


def data_dir(explicit: str = "") -> str:
    """경로(맵) CSV 폴더 — mapping 이 쓰고 driving·prompt 가 읽는다."""
    return _resolve(explicit, 'WHITE806_DATA_DIR', 'gps_data')


def record_dir(explicit: str = "") -> str:
    """주행 기록 CSV 폴더 — record 가 쓴다."""
    return _resolve(explicit, 'WHITE806_RECORD_DIR', 'ros2bag')
