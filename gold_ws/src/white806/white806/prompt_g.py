...
# -*- coding: utf-8 -*-
"""
prompt_g.py ― GUI 메인화면 [white806]
════════════════════════════════════════════════════════════════════════════════
 prompt.py 와 ★기능이 같다★ — 조작 수단만 CLI 에서 tkinter 창으로 바뀐 것이다.
════════════════════════════════════════════════════════════════════════════════
    ros2 run white806 prompt_g          (one_launch.py 를 띄운 뒤 별 터미널에서)
...
class App:
    ...
    def __init__(self, node: PromptGuiNode):
        ...
        self.root = tk.Tk()
        self.root.title('white806 — 매핑 / 주행')
    ...
    def _startable(self):
        ...
        if self.node.auto_mode is None:
            ...
            return False
        if self.node.state == 'ESTOP':
            self.say('🚨 E-STOP 중 — 하드웨어로 해제해야 한다')
            return False
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().error(
            f"창을 열 수 없다({e}) — DISPLAY 가 없으면 `ros2 run white806 prompt` 를 쓸 것")
    finally:
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()
