...
# [white2] 단위 환산의 단일 소유자(1/5카 규약). 엔코더/조향 상수는 여기서만 정의한다.
...
# [white2] 1/5카 원본값 0.8 유지 — /cmd_vel_raw 가 m/s 그대로라 kasa 처럼 펄스
#   반올림에서 0(정지)으로 떨어질 위험이 없다(그 걱정은 kasa 이식 때 생긴 것).
MIN_SPEED_FLOOR          = 0.8
...
GAIN_TABLE = [
    ...
    (2.2,     2.3,  0.20,  0.21),  # LFD 3.2  : 차량 최고속도(8km/h=2.222m/s) 바로 아래
    # [white2] kasa 이식 때 추가된 2.65~4.42 행(미검증 추정값)은 삭제했다 — 이 차량은
    #   물리적으로 2.222 m/s 를 못 넘으므로 그 구간은 애초에 쓰일 일이 없다.
...
LFD_TABLE = [
    ...
    (2.2,     3.2),   # ω_n 0.97  차량 최고속도(2.222 m/s) 바로 아래
    # [white2] kasa 이식 때 추가된 2.65~4.42 행(실차 미검증 추정값)은 삭제했다 — 이
    #   차량은 물리적으로 2.222 m/s 를 못 넘으므로 그 구간은 애초에 쓰일 일이 없다.
...
class DrivingNode(Node):
    ...
    # ── 엔코더/속도 환산 상수 ──────────────────────────────────────────────
    # [white2] 값은 white2/car_units.py 가 소유한다(1/5카 규약) ★ 여기서 리터럴을
    #   다시 쓰면 gps_imu / sensor_monitor 와 어긋난다.
    #     1/5카(motor.py) : 0.8482 m 둘레 / 300틱 1회전 / 10ms 창 → 1틱 = 0.283 m/s
    WHEEL_CIRCUMFERENCE   = ku.WHEEL_CIRCUMFERENCE_M      # 0.8482 m
    TICKS_PER_REV         = float(ku.ENCODER_COUNTS_PER_REV)   # 300 (부호 있는 단일값)
    DT_ENC                = ku.PULSE_WINDOW_S             # 0.010 s (아두이노 속도PID 주기)
    # [white2] 1/5카 원본값 0.05 로 복원 — 아두이노가 E 프레임을 촘촘히(10ms) 보내
    #   alpha 가 작아도 충분히 매끄럽다(0.15 는 kasa 의 20Hz·거친 양자화에 맞춰 늘린 값).
    ...
    def __init__(self):
        ...
        # ── 차량/속도 파라미터 (1/5카 원본 제원) ───────────────────────────
        self.vehicle_length = ku.WHEELBASE_M       # 휠베이스 0.73 [m]
        self.STEER_MAX_DEG  = float(ku.STEER_MAX_DEG)   # ROS측 조향 클램프 (motor.py 도 ±21°)
        self.STEER_AUTHORITY_FULL_SPEED = 0.5      # [v6.6] 이 속도[m/s]↑서 조향 full 권한(저속 램프)
        # 물리 최소 회전반경 = L/tan(steer_max) = 0.73/tan(21°) ≈ 1.90m
        ...
        #
        # [white2] kasa 이식 때 이 기본값을 4.42(5펄스 규약)까지 올렸던 문단은 삭제했다 —
        #   1/5카는 펄스 규약이 아니라 m/s 를 직접 보내고(motor.py), 물리 최고속도도
        #   8km/h(2.222 m/s) 뿐이라 그 위로 올릴 근거가 없다. 1.8 = 위 실측(2.11 m/s)의
        #   85% 헤드룸 그대로다.
        self.declare_parameter("max_speed_ms", 1.8)    # 상한[m/s] — 1/5카 실측 헤드룸 기준
        self.max_speed_ms   = max(0.3, float(self.get_parameter("max_speed_ms").value))
        # motor.py 프로토콜 상한(5.0 m/s)을 넘는 값은 거기서 잘리므로 여기서 미리 막는다
        self.max_speed_ms   = min(self.max_speed_ms, ku.MAX_SPEED_MS_LIMIT)
        self.safe_stop_speed = 0.08                # 정지로 간주하는 미세속도
        # ── 하한 override ──────────────────────────────────────────────────────
        #   min_speed_ms 는 원래 상한에 비례해 산출된다(MIN_SPEED_RATIO = 1/2.8).
        #   저속 시험·수집에서는 하한을 직접 고정하고 싶을 수 있으므로 override 를 연다.
        #     0 (기본) = 자동(상한 비례 산출, 종전 동작 그대로)
        #     >0       = 그 값을 하한으로 사용 (MIN_SPEED_FLOOR 와 상한 사이로 클램프)
        #   예: ros2 param set /driving_node min_speed_ms 0.8
        ...
        # [0713] 실측으로 4.4 를 기각하고 4.0 으로 되돌렸다(v=2.8 동작점, 표값 자체를
        #   올린 실험). LFD_TABLE 최상단(2.2행, 3.2m)보다 높은 천장을 유지해야 그
        #   실측이 죽지 않으므로 max(4.0, ...) 형태로 둔다.
        ...
        # [white2] 1/5카 실측값(v6.7.1)으로 복원 — 서보 스케일/링키지/슬립으로 인한
        #   15% 부족조향 보정. 재측정법: 로스백에서 cmd 조향↔IMU 요레이트 상호상관 기울기.
        ...
        # [white2] 1/5카 실측값(v6.7.2)으로 복원 — kasa 이식 때 '미측정'으로 1.0/1.0 에
        #   방치됐던 값을 되돌린다.
        self.STEER_PLANT_GAIN_L = 0.85     # 좌회전(δ_ctrl>0) 실효게인
        self.STEER_PLANT_GAIN_R = 0.82     # 우회전(δ_ctrl<0) 실효게인
        ...
        # [white2] /cmd_vel_raw 도 linear.x = m/s 그대로다(motor.py 가 직접 받는다) —
        #   kasa 이식 때 생긴 '펄스 목표값' 개념은 1/5카에는 없다. /cmd_vel_drive(카메라
        #   게이트 경유)든 /cmd_vel_raw(motor 직결)든 항상 같은 단위라 분기가 필요 없다.
        self.get_logger().info(f"[white2] 출력 토픽 {_cmd_topic} → linear.x 단위 = m/s")
    ...
    def cb_encoder(self, msg: Int32):
        """/encoder — [white2] motor.py 가 그대로 실어 보내는 ★부호 있는 틱/10ms 창★.
        음수가 후진을 뜻하며, 자율주행 중에도 그대로 유효하다(kasa 처럼 후진을
        수동조종 전용으로 막지 않는다)."""
        raw = float(msg.data)
    ...
    def _on_set_parameters(self, params):
        ...
        for p in params:
            if p.name == "cam_lat_enable":
                ...
                # [white2] motor.py 프로토콜 상한(5.0 m/s)을 넘는 값은 거기서 잘린다
                ...
                # 하한 override 런타임 조정 — 0 이면 상한 비례 산출로 되돌린다.
    ...
    def publish_cmd(self, speed_ms: float, steer_deg: float):
        ...
        msg = Twist()
        # [white2] linear.x 는 항상 m/s(motor.py 가 직접 받는다) — kasa 의 펄스 환산 없음.
        msg.linear.x  = speed_ms
        # [white2] 1/5카 규약(+좌/−우)은 내부 제어 규약과 이미 같은 부호라 뒤집지 않는다
        #   (kasa 이식 때 생긴 to_ros_steer() 반전은 kasa B보드 부호에 맞추기 위한 것이었다).
        #   steer_out 은 위에서 이미 ±STEER_MAX_DEG 로 클램프했으므로 그대로 발행한다.
