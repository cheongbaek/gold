...
try:
    from white2.route_remodeler import remodel_route
except Exception:
    try:
...
class MappingNode(Node):
    ...
    def __init__(self):
        ...
        # 주행 방향. 첫 /encoder 수신 전 기본값(전진) — 이후 cb_encoder 가 부호로 갱신한다.
        self.direction = 1

        # ── ★수동조종 실계측 3종 (수집 라벨)★ ──
        # [white2] ①/throttle_pedal·/drive_pulse_cmd 는 kasa 페달 패스스루 전용 토픽이라
        #   1/5카(motor.py)에는 없다 — 항상 0 으로 남는다(호환을 위해 컬럼은 유지).
        self.throttle_pulse = 0    # ① /drive_pulse_cmd  (1/5카에는 발행자 없음 — 항상 0)
        self.wheel_pulse    = 0    # ② /encoder          실제 주행 틱 (부호 있는 단일값)
        # ③ /steer_angle_measured  실측 조향각 [deg]
        #   ★부호는 + 좌 / − 우 (1/5카 규약)★ — motor.py 값을 그대로 통과시킨다.
        self.steer_measured = 0.0
        self.throttle_raw   = 0    # 부수 : 1/5카에는 페달 raw 토픽이 없음 — 항상 0
        self.auto_mode      = None # 부수 : 1/5카에는 물리 모드 스위치가 없음 — 항상 None
        self.estop          = False  # 부수 : 1/5카에는 /estop 토픽이 없음 — 항상 False
    ...
    def cb_encoder(self, msg: Int32):
        """② 실제로 돈 주행 틱 (부호 있는 단일값, 10ms 창, motor.py).
        [white2] 1/5카는 후진이 있다 — kasa 처럼 +1 로 고정하지 않고 부호로 읽는다."""
        raw = int(msg.data)
        self.wheel_pulse = abs(raw)
    ...
    def start_mapping(self):
        ...
        self.csv_writer.writerow([
            ...
            # ③ 실측 조향각 [deg, ★− 좌 / + 우★] (/steer_angle_measured) = steer 와 동일값
            #    ⚠️ 1/5카가 기록한 구맵의 steer 컬럼은 반대 부호(+좌)다 — 구맵을
            #       tool/map_check.py 로 분석할 때 좌우가 뒤집혀 보인다(주행에는 영향 없음:
            #       driving.py 는 CSV 의 steer 컬럼을 읽지 않는다).
            "steer_measured",
        ...
        # [white2] 1/5카(motor.py)는 kasa 의 B보드 D5 물리 모드 스위치·/vehicle_mode 가
        #   없다 — auto_mode 는 항상 미수신(None)이라 더 이상 경고할 근거가 못 된다.
        if self.auto_mode is None:
            self.get_logger().info(
                "ℹ️ /vehicle_mode 없음(1/5카 기본 상태) — throttle_pulse/wheel_pulse/"
                "steer_measured 는 motor 노드의 실측값을 그대로 기록합니다."
            )
