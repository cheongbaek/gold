...
#  ★BEV 사다리꼴 기본값 [2026-08-24 갱신]★  좌상, 우상, 우하, 좌하 (보정된 원본 화면 좌표)
#
...
def declare_params(node):
    ...
    #   ★앞범퍼가 BEV 의 몇 번째 행인가 = 거리 0 의 기준★ 차체가 화면 하단을 가려서
    #   범퍼가 사다리꼴 밑변보다 더 아래(=차 쪽)에 있으면 bev_h 보다 큰 값을 준다.
    #   실측 절차는 STOPLINE_TEST.md 단계 2-B.
    #
    #   ★[2026-08-25 실측] 480(=bev_h, 근거 없는 기본값) → 645★
    #     · 줄자 실측: ★앞범퍼 → 사다리꼴 밑변 = 1.20 m★ (2-B 가 시키는 그 측정이다)
    #     · 밑변이 카메라 앞 1.49 m 이므로 범퍼는 카메라 앞 0.29 m 다
    #     · 그 지면점의 원본 행은 3414 (화면 밖) → 중앙열 사영식으로 BEV 행 645
    #   ⚠️ 2-B 의 선형 근사(`480 + 거리÷1px당m`)로는 623 이 나온다. 1.20 m 를
    #      한 축척으로 밀기에는 이 BEV 가 너무 비선형이라 ★사영식으로 풀었다★.
    #      중앙열 사영식 r(v) = (−1.4853·v + 965.47)/(−0.002158·v + 1) 의 극점은
    #      v=463 (화면 위쪽)이라 아래쪽 외삽은 발산하지 않는다 — 확인하고 쓴 값이다.
    #
    #   ⚠️ ★이 값이 sl_px 의 도달 범위를 통째로 정한다★ 645 이면 BEV 밑변이
    #      sl_px=165 이므로 ★sl_px 는 165 밑으로 절대 못 내려간다★. 차체 가림까지
    #      넣으면 실효 하한은 sl_px=245(범퍼 앞 1.86 m)다. 문턱을 그보다 낮게 두면
    #      영영 발화하지 않는다(종전 sl_brake2_px=60 이 그 상태였다).
    d('bev_bumper_y_px', 645.0)
    #   BEV 한 픽셀이 몇 m 인가. ★제어에는 쓰지 않는다★ — 0 이 아니면 HUD·로그에
    #   참고 미터를 함께 찍어 준다(픽셀 문턱을 미터로 감 잡을 때만 쓴다).
    #
    #   ★[2026-08-25 실측] 0.006 → 0.0082★ 종전 값은 cam_testbed 의
    #   pixel_to_meter_bev 를 이름만 바꿔 옮겨온 것이었는데 ★그 값은 이 사다리꼴에서
    #   잰 것이 아니다★. night_a 영상(cam-20260824_201445.mp4)의 ★실제 정지선★ 으로
    #   기하를 풀었다(그 영상에 정지선이 없다는 종전 설명은 틀렸다 — 프레임 575~621).
    #
    #   ── 어떻게 풀었나 ────────────────────────────────────────────────
    #     ① 소실점: 횡단보도 줄무늬 에지 28개 → (939.7, 521.5). 지평선 행 vh=521.5
    #     ② 자(ruler): 정지선을 ★45 cm(실측)★ 짜리 고정 길이 자로 쓴다. 접근하는
    #        16프레임에서 원·근 에지를 half-max 로 재고 아래 모델에 맞췄다:
    #            두께(원본px) = (T/P)(fa−vh)(fb−vh) + β
    #        β 는 ★블러 편향★ 이다 — 이걸 안 넣으면 원거리에서 두께가 부풀어
    #        축척이 20% 흐른다. 적합 RMS 0.39px (두께 15.7~68.7px 구간).
    #     ③ 결과: ★Z(v) = 830.6/(v − 521.5) [m]★ , 카메라 높이 H = P/fy = 0.893 m
    #
    #   ── 교차검증 ─────────────────────────────────────────────────────
    #     · 이 기하로 횡단보도 주기를 역산하면 0.790 m → ★40+40cm 규격과 1% 일치★
    #       (정지선 45cm 과 횡단보도 40cm 이 서로 맞물려 확인된다)
    #     · 접근속도 1.92 m/s — 이 차량에 타당하다
    #   ⚠️ 자 하나로는 ★vh 와 H 를 못 가른다★ (vh 510~540 에서 잔차가 거의 평평하고
    #      H 만 0.93→0.83 으로 흐른다). 그래서 vh 는 반드시 소실점에서 따로 잡는다.
    #      남은 불확실성은 ±10% 쯤이고, 그것이 아래 값들의 오차 한계다.
    #
    #   ⚠️ ★이 상수는 «국소 축척»이 아니다★ m_txt() 가 하는 계산은 `sl_px × px_to_m`
    #     이므로, 필요한 것은 ★원점(범퍼)부터의 할선 기울기★ 다. bev_bumper_y_px 가
    #     645 라 원점이 BEV 밖(밑변보다 1.20 m 뒤)에 있고, 그래서 'BEV 깊이 ÷ 480행'
    #     같은 평균값을 쓰면 통째로 빗나간다 — 실제로 0.010 을 넣었더니 표시가
    #     +16~38% 부풀었다.
    #
    #     sl_px   참 거리    0.0082 표시   오차
    #       165    1.20 m      1.35 m    +12.8%   ← BEV 밑변
    #       300    2.34 m      2.46 m     +5.1%   ← ★2단 문턱★
    #       400    3.30 m      3.28 m     −0.6%
    #       470    4.04 m      3.85 m     −4.5%   ← ★1단 문턱★
    #       645    6.18 m      5.29 m    −14.4%   ← 사다리꼴 윗변
    #
    #     0.0082 는 ★운용구간(2단~1단 문턱, 300~470)의 최소제곱 최적값★ 이다(0.00824).
    #     거기서 ±5% 안에 들고, 대신 양 끝에서는 10% 넘게 틀어진다 — 문턱을 옮기면
    #     이 상수도 그 구간에 맞춰 다시 잡을 것.
    #     ★표시 전용이라 판정은 안 흔들린다★ — 정확한 거리가 필요하면 상수가 아니라
    #     위 Z(v) 로 풀 것.
    #
    #   ⚠️ 이 값이 틀리면 판정은 안 흔들려도 ★사람이 문턱을 잘못 정한다★ — 종전에는
