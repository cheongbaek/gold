...
// ══════════════════════════════════════════════════════════════════════════
//  주행은 ★사람이 페달로★ 한다 (B보드 D5 = 수동조종). 그 페달은 이제 펄스가
//  아니라 ★직접 PWM★ 으로 A보드에 내려간다 — arduino.py 가 페달 개도량을
//  manual_pwm_min~manual_pwm_max 에 비례시킨다(그쪽 헤더 (2) 분기 / 2026-08-25).
//  즉 ★밟은 만큼 나간다★ 가 이 시험의 주행 모델이고, 이 노드는 그 위에
//  '앞이 막히면 세운다' 하나만 더한다.
//
...
//  속도를 실제로 묶어야 하면 곳은 하나다: 런치의 ★manual_pwm_max★ (arduino.py 가
//  페달 개도량을 PWM 에 대응시키는 유일한 지점). 여기서는 실측 속도를 ★그냥
//  보여줄 뿐★ 이다 — 판정에 쓰지 않는다.
//
...
public:
  PedalDriveNode() : Node("pedal_drive_node") {
    ...
    RCLCPP_INFO(
        ...
        "     ★속도 제한 없음★ 페달이 곧 속도다 (묶는 곳은 런치의 "
        "manual_pwm_max 하나뿐). 이 노드는 속도를 판정하지 않는다\n"
