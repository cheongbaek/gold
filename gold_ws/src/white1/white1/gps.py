...
GATE_ENABLE       = True
# ══════════════════════════════════════════════════════════════════════════════
#  ★★ [2026-09-13] 8.0 → 11.2 — 게이트가 제동검차 계측을 막고 있었다 ★★
# ══════════════════════════════════════════════════════════════════════════════
#  ★2026-09-13 제동검차 로그가 이 상수 하나 때문에 30 km/h 를 '못 넘긴 것' 이 됐다.★
#  실제로는 넘겼다 — fix 원값(quality 4, σ 0.012, 전 구간 RTK Fixed)으로 직접 재면
#  최고 ★33.37 km/h★(t=12.89 s)이고 30 km/h 초과가 5.0 초 이어졌다. 그런데
#  `/gps_fused` 의 속도는 ★29.89 km/h 에서 멈췄다★:
#
#      d_max = min(v·dt + ½·a·dt², 8.0·dt) + noise = 8.0×0.2 + 0.12 = ★1.720 m★
#      실제 이동 1.722 m (= 30.99 km/h) 부터 전부 기각 — 21 fix 연속
#
#  `GATE_MAX_REJECT_N`(5) 탈출구가 5개마다 1개씩만 통과시켜, 융합 위치가 계단식으로
#  점프하고(45.77 → 38.41 → 25.78 → 14.79 m) 속도는 아예 안 나왔다. 그 결과
#  braketest 의 제동 결과표가 ★`v0 nan km/h, a nan/nan`★ 으로 비었다 —
#  ★재려던 값 자체를 게이트가 지웠다.★
#
#  ★값의 근거★ 8.0 은 "5펄스 4.42 의 1.8배" 로, ★4~5펄스가 상한이던 시절★ 것이다.
#  지금 제동검차는 15펄스로 출발해 30 km/h 를 넘겨야 하고(사용자 지시), 운용 상한도
#  10펄스(31.8 km/h)다. 11.2 m/s = ★40.3 km/h★ 는 그 위에 오버슈트 여유를 둔 값이다:
#      10펄스 31.8 · 15펄스 47.7(도달 전에 순항으로 내린다) · 실측 최고 33.4
#  ⚠️ 이 게이트를 열어도 ★큰 튐은 아래 두 항이 잡는다★ — 가속도 게이트(6.0 m/s²)가
#     한 샘플 점프를 여전히 묶고(dt=0.2 에서 +0.12 m), 방향 게이트가 옆걸음을 잡는다.
#     실제로 이 로그의 기각 21건은 ★전부 속도항★ 이었다(가속도·방향 위반 0건).
GATE_SPEED_MAX_MS = 11.2   # [m/s] = 40.3 km/h. 이 차가 낼 수 없는 속도
#   ★자율주행 상한이 아니라 차량 쪽에 여유를 두고 잡는다★ 이 노드는 수동 계측
#   런치에서도 돌고, 사람이 몰면 더 빠를 수 있다. 게이트가 정상 주행을 버리는 것이
#   튐을 놓치는 것보다 나쁘다 — 넉넉하게 두고 아래 두 항이 실제로 조인다.
