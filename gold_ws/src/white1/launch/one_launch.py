...
════════════════════════════════════════════════════════════════════════════════
 ★terrain 열 규약 — 사람이 손으로 적는 '구간 지정' 열★
════════════════════════════════════════════════════════════════════════════════
    값          뜻                                     누가 처리하나
    ─────────  ────────────────────────────────────   ───────────────────────
    ...
    traffic_light 는 종전대로 그것이 신선할 때만 허락으로 읽는다.
    빨간불이 아니면(초록·미검출·불확실) ★일시정지 없이 통과★ 한다.

        driving ──/tl_permit(T 구간에서만 True)──▶ traffic_light
        driving ◀──/tl_brake_req(0 또는 2)─────── traffic_light   (max 로 합성)

    ⚠️ ★구간을 벗어나면 제동이 즉시 풀린다★ (사용자 결정) 그래서 T 는 정지선 뒤로
    ★정지거리만큼 더★ 이어져 적어야 한다 — 4펄스면 약 3m, 6펄스면 약 8m.
    경로를 고를 때 driving 이 구간 길이를 재서 모자라면 경고한다.

    ⚠️ ★T 가 하나도 없는 CSV 에서는 신호등이 전 구간에서 개입하지 않는다★
    종전 경로가 전부 여기에 해당한다. 그 사실도 경로 선택 시 로그에 남는다.
